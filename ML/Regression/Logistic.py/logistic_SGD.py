# THE PARAMETERS FOR LOGISTIC REGRESSION IN THIS CODE ARE OBTAINED USING 'STOCHASTIC GRADIENT DESCENT'
import numpy as np
...
class Logistic_Regression:
    ...
    def fit(self, X, y, theta_ini, learning_rate = 0.1, max_iter = 1000):
        ...
        for i in range(max_iter):
            ...
            for j in range(n):
                grad = self._grad_cost(theta, np.array([X_shuffled[j]]), np.array([y_shuffled[j]]))
                theta -= learning_rate * grad # decaying learning rate for better convergence near point of minima
            epoch_costs.append(self._cost_func(theta, X_b, y))  # full-batch cost    

        self._parameters = theta
        self._cost = np.array(epoch_costs)
...
if __name__ == '__main__':
    ...
    print('The model is being trained ... pls wait')
